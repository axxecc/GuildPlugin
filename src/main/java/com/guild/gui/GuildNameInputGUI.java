...
public class GuildNameInputGUI implements GUI {
    ...
    private void handleInputName(Player player) {
        ...
        // 发送输入提示
        sendMessage(player, "&6请输入新的工会名称:");
        sendMessage(player, "&7当前名称: &f" + currentName);
        sendMessage(player, "&7输入 &c取消 &7来取消操作");
        sendMessage(player, "&7支持颜色字符，例如: &a&l绿色粗体 &7或 &c&o红色斜体");
        sendMessage(player, "&7注意：工会名称不能与其他工会重复");
    }
    ...
    public void handleInputComplete(Player player, String input) {
        ...
        if (cleanName.length() < 2) {
            sendMessage(player, "&c工会名称至少需要2个字符（不包括颜色字符）！");
            plugin.getGuiManager().openGUI(player, this);
            return;
        }

        if (cleanName.length() > 16) {
            sendMessage(player, "&c工会名称不能超过16个字符（不包括颜色字符）！");
            plugin.getGuiManager().openGUI(player, this);
            return;
        }
        ...
        if (newName.equalsIgnoreCase(currentName)) {
            sendMessage(player, "&c新名称与当前名称相同！");
            plugin.getGuiManager().openGUI(player, this);
            return;
        }
        ...
        if (!cleanName.matches("^[\\u4e00-\\u9fa5a-zA-Z0-9]+$")) {
            sendMessage(player, "&c工会名称只能包含中文、英文和数字！");
            plugin.getGuiManager().openGUI(player, this);
            return;
        }
        ...
        plugin.getGuildService().getGuildByNameAsync(newName).thenAccept(existingGuild -> {
            if (existingGuild != null) {
                ...
                CompatibleScheduler.runTask(plugin, () -> {
                    sendMessage(player, "&c工会名称 &f" + newName + " &c已被使用！");
                    plugin.getGuiManager().openGUI(player, this);
                });
                return;
            }
            ...
            plugin.getGuildService().updateGuildAsync(guild.getId(), newName, guild.getTag(), guild.getDescription(), player.getUniqueId())
                    .thenAccept(success -> {
                        CompatibleScheduler.runTask(plugin, () -> {
                            if (success) {
                                // 更新成功
                                sendMessage(player, "&a工会名称修改成功！");
                                sendMessage(player, "&7新名称: &f" + newName);
                                ...
                                // 更新失败
                                sendMessage(player, "&c工会名称修改失败！请重试");
                                plugin.getGuiManager().openGUI(player, this);
                            }
                        });
                    });
