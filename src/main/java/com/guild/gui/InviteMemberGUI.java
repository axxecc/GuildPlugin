...
public class InviteMemberGUI implements GUI {
    ...
    public Component getTitle() {
        String rawTitle = "&6邀请成员 - 第" + (currentPage + 1) + "页";
    ...
    private void handleInvitePlayer(Player inviter, Player target) {
        ...
        plugin.getGuildService().getGuildMemberAsync(target.getUniqueId()).thenAccept(member -> {
            if (member != null) {
                CompatibleScheduler.runTask(plugin, () -> sendMessage(inviter, InviteMessageUtils.formatAlreadyInGuild(plugin, target.getName())));
                return;
            }
            ...
            plugin.getGuildService().sendInvitationAsync(guild.getId(), inviter.getUniqueId(), inviter.getName(), target.getUniqueId(), target.getName())
                    .thenAccept(success -> {
                        ...
                        CompatibleScheduler.runTask(plugin, () -> {
                            if (success) {
                                sendMessage(inviter, InviteMessageUtils.formatInviteSent(plugin, inviter, target));

                                // 给被邀请者发送标题与带 inviter 的邀请信息
                                sendMessage(target, InviteMessageUtils.formatInviteTitle(plugin));
                                sendMessage(target, InviteMessageUtils.formatInviteReceived(plugin, inviter, guild));
                            } else {
                                sendMessage(inviter, InviteMessageUtils.formatInviteFailed(plugin));
                            }
                        });
                    });
        });
